...
- The popup refreshes tab and queue status every three seconds. The optimizer can be injected on demand when a selected ChatGPT tab does not yet have the content script; Gemini tabs are selectable for queue but not for optimizer (`popup.js`).
